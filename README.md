...
Antigravity (and other MCP-compatible AI agents) can now directly use Vivian's graph-aware capabilities to analyze, search, and read your codebase.

## What changed?
1. **Added Dependency:** We added `mcp` to `Server/requirements.txt`.
2. **MCP Server Endpoint:** We created `Server/mcp_server.py`, which is a FastMCP application.

## Available Tools
The new MCP server exposes three tools natively:
...
*   **`search_graph_nodes`**: Searches Vivian's parsed graph nodes (functions, classes, interfaces, records, etc.) for a specific name or text.
...
Antigravity is designed to consume MCP servers over `stdio`. Since the `mcp_server.py` uses `mcp.run(transport='stdio')`, you can simply add the following command to your Antigravity (or Cline) MCP configurations:
...
```json
{
  "mcpServers": {
    "vivian-graph": {
      "command": "python",
      "args": ["/absolute/path/to/Vivian/Server/mcp_server.py"],
      "env": {}
    }
  }
}
...
> [!TIP]
> Make sure the python path resolves to the virtual environment `venv` where `mcp` and Vivian's requirements are installed! You might want to use `/absolute/path/to/Vivian/Server/venv/bin/python`.
