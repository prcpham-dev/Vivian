...
> [!IMPORTANT]
> **Prerequisite — Scan your workspace first.**
> Vivian's MCP tools read from a pre-built cache. Before your agent can use any tool, you must open the **Vivian Graph panel** in VS Code and click **Scan** to build the graph for your workspace.
> If you skip this step, all MCP tool calls will return an error asking you to scan first.
...
4. Paste that JSON **manually** into your agent's MCP settings file (see locations below).

> [!IMPORTANT]
> **This is a manual step.** Vivian copies the config to your clipboard, but you must open your agent's config file yourself and paste it in. Vivian cannot write to external agent config files automatically.
