...
Return JSON array of vulnerabilities found. Each entry MUST include: line (integer line number, not null), type (string - e.g., "Weak Hash Algorithm", "Insecure SSL/TLS"), severity (high/medium/low), description (string - concise one sentence explanation), location (file path).
...
[
  {{
    ...
    "severity": "high",
    "description": "MD5 hash function used at line 42, which is cryptographically broken and vulnerable to collision attacks, making it unsuitable for security purposes.",
    "location": "{file_path}"
  }}
]

If no vulnerabilities found, return empty array []. Be specific about what cryptographic issues you identify and ALWAYS include the line number. Provide concise one-sentence descriptions explaining the vulnerability and its security risks."""
    else:
        user_prompt = f"""Analyze this file for cryptographic failure vulnerabilities:
...
If no vulnerabilities found, return empty array []. Be specific about what cryptographic issues you identify. Provide concise one-sentence descriptions explaining the vulnerability and its security risks."""
